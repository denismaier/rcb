...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt">
  <title>JNDF JATS Basis-Validierung</title>
  ...
  <pattern id="doi-format">
    <rule context="article/front/article-meta/article-id[@pub-id-type='doi']">
      ...
      <assert test="starts-with(., 'https://doi.org/10.36950/jndf.')">
        Ungültiger DOI: <value-of select="." />. DOI muss mit 'https://doi.org/10.36950/jndf.' beginnen.
      </assert>
      <!-- Prüft, dass der DOI auf eine Zahl endet (Jahreszahl.Nummer) -->
      <assert test="matches(., 'https://doi.org/10.36950/jndf.[0-9]+\.[0-9]+$')">
        Ungültiger DOI: <value-of select="." />. DOI muss auf eine Zahl enden (z.B. https://doi.org/10.36950/jndf.2026.1).
      </assert>
    </rule>
  </pattern>

  <!-- Test-Regel: Wenn die aktiviert ist, bricht der Build immer ab -->
  <!-- <pattern id="test-always-fail">
    <rule context="article">
      <assert test="false()">TEST: Diese Regel schlägt immer fehl (zu Testzwecken)</assert>
    </rule>
  </pattern> -->
